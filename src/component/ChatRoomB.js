...
const ChatRoomB = () => {
  ...
  useEffect(() => {
    ...
    clientRef.current.onConnect = (frame) => {
      clientRef.current.subscribe(`/topic/chatB/${roomName}`, (message) => {
  ...
  const sendMessage = () => {
    ...
    clientRef.current.publish({
      destination: `/app/chatB/${roomName}`,
      headers: {},
  ...
  const enter = () => {
    clientRef.current.publish({
      destination: `/app/helloB/${roomName}`,
      headers: {},
      body: writer,
    });
