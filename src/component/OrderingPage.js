...
useEffect(()=>{ 
          ...
          function callback(response) {
            ...
            if (success) {
              ...
              const orderingData = 
              {
                ...
                orderDate: null,
                salePrice: orderProduct.sale?orderProduct.discountPrice:orderProduct.price,
                amount: orderAmount,
  ...
  //쇼핑몰 페이지에서 받아온 productId, amount로 주문 상품 정보 저장 
  const [orderProduct, setOrderProduct] = useState({});
  const orderAmount = sessionStorage.getItem('productAmount');
  const orderProductId = sessionStorage.getItem('productId');

  useEffect(()=>{ 
    callApi('http://localhost:8080/shopApi/getProduct/'+orderProductId, 'GET')
    .then((response)=>{return response.json();}).then((json)=>{
        setOrderProduct(json);
    })
    .catch((status)=>console.log(status));
  }, [])
...
if(orderProduct!=null){
  total = orderAmount*(orderProduct.sale?orderProduct.discountPrice:orderProduct.price);
}
...
<div className='my-page'>
      ...
      <tbody>
        <tr>
          ...
          <td>{orderAmount}</td>
          <td>{priceToString(orderAmount*(orderProduct.sale?orderProduct.discountPrice:orderProduct.price))}</td>
        </tr>
      </tbody>
    </Table><br/>
