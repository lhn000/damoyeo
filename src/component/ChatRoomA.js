...
const ChatRoomA = () => {
  ...
  useEffect(() => {
    ...
    clientRef.current.onConnect = (frame) => {
      clientRef.current.subscribe(`/topic/chatA/${roomName}`, (message) => {
  ...
  const sendMessage = () => {
    ...
    clientRef.current.publish({
      destination: `/app/chatA/${roomName}`,
      headers: {},
  ...
  const enter = () => {
    clientRef.current.publish({
      destination: `/app/helloA/${roomName}`,
      headers: {},
      body: writer,
    });
